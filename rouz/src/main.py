...
def main(holidays=None, multistep=False):
    ...
    model_names = {'Baseline': models.Baseline(label_index=column_indices['DA_price']),
                   'Linear': models.Linear()}
    for u in range(1, 61, 10):
        model_names[f'Dense (units={u})'] = models.DenseModel(units=u)
    
    deep_dense_units = {2: [40, 20],
                        3: [40, 20, 10],
                        4: [40, 20, 10, 5]}
    for ilayer in range(2, 5, 1):
        model_names[f'Deep Dense {ilayer} layers'] = models.DeepDenseModel(num_layers=ilayer,
                                                                          units=deep_dense_units[ilayer])

    model_names['Conv no dense'] =  models.ConvolutionalModel(c_window=CONV_WIDTH,
                                                                     add_dense=False,
                                                                     filters=6)

    model_names['Conv with dense (units=6)'] = models.ConvolutionalModel(c_window=CONV_WIDTH, add_dense=True, units=6, filters=6)

    model_names['LSTM'] = models.RecurrentModel(layer='LSTM', units=32)
    model_names['GRU'] = models.RecurrentModel(layer='GRU', units=32)
    model_names['Dense and LSTM'] = models.RecurrentModel(layer='LSTM', units=8, add_dense=True)
    ...
    for model_name, model in model_names.items():
        ...
        if model_name == "Baseline":
            ...
            fig = wide_window.plot(model, max_subplots=3, 
        ...
        model_save_loc = pathlib.Path(f"../models/dat_set_rouz/{output_type}_step/{tmp}/")
        ...
        histories[model_name] = models.compile_and_fit(model, window=window, 
                                                   patience=PATIENCE,
        ...
        for x, y in validation_dataset:
            ...
            y_pred = model.predict(x, verbose=0)
            y_pred = y_pred * da_price_std_train + da_price_mean_train
            ierr, inum = get_rmse(y_pred, y_tru, is_residual, index=column_indices['DA_price'])
            root_mean_sq_err_val += ierr
            num += inum 
        ...
        fig = window.plot(model, max_subplots=3, 
                        title=f'{model_name}', 
                        colors=[input_color,label_color, prediction_color])
        ...
        if model_name == "Linear":
            ...
            fig, ax = plt.subplots(1,1, figsize=(16,9), 
                                   gridspec_kw={'left':0.05,'right':0.98,
                                                'top':0.95, 'bottom':0.35})
            features = [[x,y] for x, y in zip(train_df.columns.values, model.layers[0].kernel[:,0].numpy())]
            ...
            ax.bar(x = range(len(train_df.columns)),
                    height=y)#model.layers[0].kernel[:,0].numpy())
    ...
    ax.set_xticks(ticks=x, labels=perf.keys(),
               rotation=90)
